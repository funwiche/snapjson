class t extends HTMLElement {
  ...
  constructor(t) {
    super(),
      (this.version = "2.4.0"),
      (this.config = {
        markedUrl: "/scripts/marked.js",
        prismUrl: [
          ["/scripts/prism.js", "data-manual"],
          "/scripts/prism-autoloader.js",
          "/scripts/prism-toolbar.js",
          "/scripts/prism-copy-to-clipboard.js",
        ],
        cssUrls: [
          "/styles/markdown.css",
          "/styles/highlight.css",
          "/styles/prism-toolbar.css",
        ],
        hostCss:
  ...
  buildStyles() {
    ...
    return `<div class="markdown-styles"><style>${
      this.config.hostCss
    }</style>${t('template[data-merge="prepend"]')}${
      t("template:not([data-merge])") ||
      e.reduce((t, e) => `${t}<link rel="stylesheet" href="${e}">`, "")
    }${t('template[data-merge="append"]')}</div>`;
  }
  async buildMd(t = {}) {
    return `<div class="markdown-body${
      t.classes ? this.arrify(t.classes).reduce((t, e) => `${t} ${e}`, " ") : ""
    }">${
      (await (async () => {
        if (!this.src) return "";
  ...
  async stampStyles(t) {
    const e = this.makeNode(t),
      ...
      r = [...this.root.children].find((t) =>
        t.classList.contains("markdown-styles")
      );
    r ? r.replaceWith(e) : this.root.prepend(e),
      await Promise.all(s.map((t) => this.onload(t))).catch((t) => {
        this.fire("zero-md-error", {
  ...
  stampBody(t) {
    const e = this.makeNode(t),
      s = [...this.root.children].find((t) =>
        t.classList.contains("markdown-body")
      );
  ...
  async render(t = {}) {
    ...
    this.fire("zero-md-rendered", { stamped: e });
  }
}
